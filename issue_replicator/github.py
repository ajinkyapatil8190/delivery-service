...
def _malware_template_vars(
    ...
    def iter_findings(
        ...
        for aggregated_finding in aggregated_findings:
            finding = aggregated_finding.finding
            malware_summary: dso.model.MalwareSummary = finding.data

            for malware_finding in malware_summary.findings:
                digest = malware_finding.meta.scanned_content_digest
                malware = malware_finding.details
                full_name = malware_finding.name

                yield malware, digest, full_name
    ...
    for version_key, findings in sorted(
        ...
        summary += (
            '\n| Malware | Digest | Full Name |'
            '\n| ---------------- | :-----: | :------: |'
        ) + ''.join(
            f'\n| {malware} | {digest} | {full_name} |'
            for malware, digest, full_name in iter_findings(findings)
        )
        summary += '\n---'
